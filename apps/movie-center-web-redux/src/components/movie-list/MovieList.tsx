...
const MovieList = ({movieList, history, location, dispatch, testName = 'MovieList_test'}: MovieListProps) => {

    const [sort, setSort] = useState(getDefaultSortModel('title', location.search));
    ...
    const loadMovies = () => {
        const sortOrder = SORT_ORDERS_BY_COLUMN.filter(csort => csort.column === sort.sortOrder)[0].sortOrder;

        dispatch(getMovies(movieList.filter, sortOrder, sort.sortDirection, 0, 1000));
    };
    ...
    const getMovieGenres = (movie: IMovie, limit = 3): string[] => {
        const mainGenre: MovieGenreModel | undefined = movie.genres.find((mg) => mg.mainGenre);
    ...
    return (
        <div data-test-name={testName}>
            <div className="main-page-container" data-test-name={testName}>
                ...
                {moviesNotLoaded
                    ? (<div><Loader /></div>) // TODO: Add app start loader (splash screen)
                    : (
                        <div>
                            <div className="list-templates">
                                <div className="list-create-link-and-no-items-container">
                                    ...
                                    <div onClick={() => createMovie()} className={'list-create-link-container' + (movies && movies.length) ? ' list-create-link-container-aligned' : ''}>
                                ...
                                {(movies && movies.length > 0) && (
                                    <table className="mat-elevation-z8 authority-list-table mat-table">
                                        <thead><tr className="mat-header-row">
                                            <th onClick={() => changeSortOrder('title')} className="sortable mat-header-cell cdk-column-name mat-column-name">
                                                <span className="icon-texts">Film</span>
                                                <span>{sort.sortOrder === 'title' && (<i className={`icons sort-icon fas fa-sort-${sort.sortArrow}`} />)}</span>
                                            </th>
                                            <th onClick={() => changeSortOrder('mainGenre')} className="sortable hide-small-screen mat-header-cell cdk-column-name mat-column-name">
                                            ...
                                            <th onClick={() => changeSortOrder('grade')} className="sortable mat-header-cell cdk-column-name mat-column-name">
                            ...
                            <div className="middle-panel-navigate-bottom-to-top" onClick={() => scrollToTop()}>
