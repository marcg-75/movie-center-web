...
const CrewPanel = ({movie, person, baseData, dispatch, testName = 'CrewPanel_test'}: CrewPanelProps) => {
    ...
    const addCrewMember = (event: any) => {
        ...
        const allCurrentPersonRolesForRole: Array<PersonRoleModel> = extractPersonRolesFromCrew(crew, role);
        const currentCrewMembers = existingMovieCrewMembersForRole(movieItem, role);
        const selectedCrewMember: PersonRoleModel | undefined = allCurrentPersonRolesForRole.find((a: PersonRoleModel) => a.id === parseInt(crewPersonRoleId, 10));
    ...
    const getSelectableCrewMembersByRole = (event: any) => {
        ...
        if (selectedRoleCode && selectedRoleCode.length && !!crew && !!movieItem) {
            const role: RoleEnum = RoleEnum[selectedRoleCode as keyof typeof RoleEnum];
            ...
            const allSelectablePersonRolesForRole: Array<PersonRoleModel> =
                extractPersonRolesFromCrew(crew, role).filter((personRole: PersonRoleModel) => {
    ...
    const getSelectablePersonsByRole = (event: any) => {
        ...
        if (persons && persons.length > 0 && selectedRoleCode && selectedRoleCode.length && !!movieItem) {
            const role: RoleEnum = RoleEnum[selectedRoleCode as keyof typeof RoleEnum];
            const currentCrewMembersForRole: CastAndCrewModel[] =
                existingMovieCrewMembersForRole(movieItem, role);
    ...
    if (crewLoading?.errors || personsLoading?.errors) {
        ...
        content = (<div>
            <ul>{crewLoading?.errors?.map((m, i: number) => <li key={i}>{m as string}</li>)}</ul>
        </div>);
    } else if (isMovieLoading || !movieItem || crewLoading?.loading || personsLoading?.loading) {
            ...
            <div>
                ...
                {environment.enableMovieInfoEdit && (
                    <form onSubmit={(e) => addNewCrewMember(e)} id="addNewCrewMemberForm">
                        ...
                        {!personsLoading || personsLoading?.loading ? (<Loader/>) : (
...
const existingMovieCrewMembersForRole = (movieItem: IMovie, role: RoleEnum): CastAndCrewModel[] => {
    switch (role) {
...
export const mapToPersonOptionElements = (persons: Array<NameEntityModel>): ReactNode[] => {
    if (!persons) {
        return [];
    }

    const options: ReactNode[] = persons.map((option: NameEntityModel, i: number) => {
        return <option key={i + 1} value={option.id}>{option.name}</option>;
    });
    options.unshift(<option key={0} value=""/>);
