...
const GeneralInfoPanel = ({movie, baseData, dispatch, testName = 'GeneralInfoPanel_test'}: GeneralInfoPanelProps) => {
    ...
    const [isMovieLoading, setIsMovieLoading] = useState(movie?.movieLoading?.loading);
    const [isBaseDataLoading, setIsBaseDataLoading] = useState(checkIfBaseDataIsLoading(baseData));
    ...
    const additionalGenresChanged = (event: any) => {
        ...
        for (let i = 0 ; i < selectedOptions.length ; i++) {
            ...
            let genre: SelectableModel | undefined = genres?.find((g: SelectableModel) => g.code === option.value);
    ...
    const studiosChanged = (event: any) => {
        ...
        for (let i = 0 ; i < selectedOptions.length ; i++) {
            let option: any = selectedOptions[i];
            let studio: NameEntityModel | undefined = studios?.find((s: NameEntityModel) => s.id === parseInt(option.value, 10));
    ...
    if (movieLoading?.errors) {  // TODO: Fyll på
        ...
        const mainGenre: MovieGenreModel | undefined = movieItem.genres?.find((mg) => mg.mainGenre);
        ...
        content = (
            <div>
                ...
                <LabelledSelect label="Genrer:" id="additionalGenres" defaultValue={currentAdditionalGenreCodes} options={genreOptions} callback={additionalGenresChanged} required={false} multiple={true} />
