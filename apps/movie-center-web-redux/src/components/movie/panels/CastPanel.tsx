...
const CastPanel = ({movie, person, dispatch, testName = 'CastPanel_test'}: CastPanelProps) => {
    ...
    useEffect(() => {
        if (!isActorsLoading && ((!actorsLoading?.loading && !actorsLoading?.loaded) || !actors?.length)) {
    ...
    const addActor = (event: any) => {
        ...
        const selectedActor: PersonRoleModel | undefined = allCurrentActors.find((a: PersonRoleModel) => a.id === parseInt(actorPersonRoleId, 10));
    ...
    const updateNewActorName = (event: any) => {
        ...
        if (actorsLoading?.errors || personsLoading?.errors) {
            ...
            content = (<div><ul>{actorsLoading?.errors?.map((m, i: number) => <li key={i}>{m as string}</li>)}</ul></div>);
            ...
            const selectablePersonOptions: ReactNode[] = mapToPersonOptionElements(selectablePersons);
