...
import {MovieFilter} from '../../../models/movie.model';
...
const ListFilterComponent = ({
    ...
    const [isVisible, setIsVisible] = useState(filterToggleDefault(componentName));
    const [isExtendedVisible, setIsExtendedVisible] = useState(extendedFilterToggleDefault(componentName));

    const toggleText = isVisible ? 'Dölj' : 'Visa';

    const toggleFilter = () => {
        localStorage.setItem(`${componentName}_filter_toggle_state`, `${!isVisible}`);  // TODO: Needed?
        setIsVisible(!isVisible);
    };

    const toggleExtended = () => {
        localStorage.setItem(`${componentName}_extended_filter_toggle_state`, `${!isExtendedVisible}`);
                        ...
                        {isExtendedFilterEnabled && (
                            <span className="filter-header-toggle-extended">
                                    <label className="toggle-container">
                                        <input type="checkbox" className="real-checkbox" value={'' + isExtendedVisible} checked={isExtendedVisible} onChange={() => toggleExtended()} />
                                        <div className="toggle-button"></div>
                                    </label>
                                    <span className="filter-extended-label" onClick={() => toggleExtended()}>Utökad</span>
                                </span>
                        )}

                        <span className="filter-header-toggle" onClick={() => toggleFilter()}>
            ...
            { isVisible && (
                <div className="filter-body">
                    ...
                    <div className="filter-actions-container">
                        <ClearListFilterItem clearFilter={clearFilter} compactMode={compactModeActions && enableSaveFilter} />
