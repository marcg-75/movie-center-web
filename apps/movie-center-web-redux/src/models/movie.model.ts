...
export const movieToString = (movie: IMovie): string => {
    ...
    if (genres) {
        movie.genres.forEach((movieGenre: MovieGenreModel) => genres += `${movieGenre.genre.name}${movieGenre.mainGenre ? '(main genre)' : ''}, `);
    }

    if (studios) {
        movie.studios.forEach((studio: NameEntityModel) => studios += `${studio.name}, `);
    }
...
export class MovieFilter extends Filter<MovieFilter> {
    ...
    static FILTER_DEFAULT_ALL_GENRES: SelectableModel = { 'code': 'ALL', 'name': 'Alla' };
    static FILTER_DEFAULT_ALL_FORMATS: SelectableModel = { 'code': 'ALL', 'name': 'Alla' };
